...
def fetch_pokemon(id_: int) -> dict:
    ...
    response: requests.Response = requests.get(
        url=f"{POKEAPI_BASE_URL}/{id_}"
    )  # запрос на синхрон библиотеку
...
async def main_async(pokemons_number: int):
    async def get_with_httpx(url: str):  # область замыкания (функция в функции)
        async with httpx.AsyncClient() as client:  # делаем запрос на асинхрон библиотеку httpx
